...
void CGameView::Render()
{
    ...
    UINT w = (UINT)avail.x; if (w < 16) w = 16;
    UINT h = (UINT)avail.y; if (h < 16) h = 16;

    Resize(w, h);

    if (m_pSRV)
        ImGui::Image((ImTextureID)m_pSRV.Get(), ImVec2((float)w, (float)h));
    else
        ImGui::TextDisabled("(No GameView SRV)");*/
...
{
    // 1. 패널 정보 미리 파악 (Begin/End 사이에서만 유효한 값들이라 여기서 체크)
    // 실제로는 Render에서 Begin 시점에 저장된 값을 써야 할 수도 있습니다.
    if (m_bMouseInScene)
    {
