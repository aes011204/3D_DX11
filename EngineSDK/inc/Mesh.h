...
public:
	virtual HRESULT Initialize_Prototype(MODEL eType, const aiMesh* pAIMesh, shared_ptr <class CModel > pModel, _fmatrix PreTransformMatrix);
...
private:
	...
	HRESULT Ready_VertexBuffer_For_Anim(const aiMesh* pAIMesh, shared_ptr<class CModel>);
public:
	static shared_ptr<CMesh> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, MODEL eType, const aiMesh* pAIMesh, shared_ptr<CModel> pModel, _fmatrix PreTransformMatrix);
