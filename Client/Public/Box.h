...
private:
    CBox(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext);
...
private:
    ...
    shared_ptr<CInventory> m_pStorageCom = { nullptr };

public:

    static shared_ptr<CBox> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext);
